<section id="section_three">
	
	<div class="sec_three_inner">
		
		<h1 class="sec_three_header">kentucky personal injury attorneys</h1><!-- section_three_header -->
		
		<span class="sec_three_description">At the law firm of Gary C. Johnson, P.S.C., we provide comprehensive client service in the aftermath of serious accidents. We work hard to maximize the financial value of your legal claims and also work closely with you and your family to help you meet the personal challenges that can result from an injury or a death in the family.</span><!-- sec_three_description -->
		
		<div id="sec_three_trigger" class="sec_three_top_content_wrapper content">
			
			<div class="sec_three_top_content">
			
				<p>Lorem ipsum dolor sit amet, consectetur adipiscing elit. Vestibulum mattis dolor sed arcu aliquet, in accumsan urna elementum. Nam lacinia quis lorem eget aliquam. Praesent lacinia nisi tellus, vitae vestibulum diam gravida quis. Class aptent taciti sociosqu ad litora torquent per conubia nostra, per inceptos himenaeos. Sed gravida metus massa, ac semper ex fringilla in. Quisque eu pharetra lectus. In at auctor mi. Cras at interdum eros. Etiam porta dui sed sollicitudin aliquam. Vivamus efficitur vel nisi et porttitor. Integer sit amet molestie eros, eu auctor odio. Phasellus aliquam mauris non sagittis venenatis.</p>

				<h2>Nam pulvinar sem in purus elementum placeratp ellentesque.</h2>
				
				<p>Quisque convallis sem at lectus tincidunt, sit amet euismod eros finibus. Duis ligula velit, cursus vitae varius quis, rutrum eget nibh. Orci varius natoque penatibus et magnis dis parturient montes, nascetur ridiculus mus. Nam accumsan convallis lorem in rhoncus. Praesent at vulputate lorem. Morbi et elementum velit. Aenean porttitor euismod est, ut sollicitudin enim. Duis pharetra velit vitae odio volutpat pellentesque. Donec faucibus porta varius. Nunc sit amet laoreet velit. Nam urna dui, ornare ut venenatis vulputate, pellentesque aliquam nibh. Sed et ligula at nibh interdum sodales. Maecenas porttitor consectetur justo, vitae aliquet turpis vestibulum eget. Aliquam feugiat vehicula porta. Aenean augue augue, molestie eu nibh egestas, ultrices suscipit magna.</p>
			
			</div><!-- sec_three_top_content -->
			
			<div class="sec_three_top_img">
				
				<div class="sec_three_top_img_inner">
				
					<img src="<?php bloginfo('template_directory');?>/images/content01_img.jpg"/>
				
					<a class="meet_attorneys_button" href="<?php the_permalink(79);?>">Meet the Attorneys</a><!-- meet_attorneys_button -->
				
				</div><!-- sec_three_top_img_inner -->
				
			</div><!-- sec_three_top_img -->
			
		</div><!-- sec_three_top_content_wrapper -->
		
		<div id="sec_three_bottom_trigger" class="sec_three_bottom_content_wrapper">
			
			<div class="sec_three_blockquote">
				
				<div class="sec_three_blockquote_inner" style="background:url(<?php bloginfo('template_directory');?>/images/light-grey-terrazzo.png) top left repeat">
					
					<div class="sec_three_blockquote_content">
						
						<div class="sec_three_quote">
							
							<strong>“We are driven by</strong><strong>the passion to work</strong> tirelessly for
							our clients to get them back on their feet and the <strong>results they deserve.”</strong>
						
						</div><!-- sec_three_quote -->
						
						<span class="blockquote_name">-Gary C. Johnson</span><!-- blockquote_name -->
						
					</div><!-- sec_three_blockquote_content -->
					
				</div><!-- sec_three_blockquote_inner -->
				
			</div><!-- sec_three_blockquote -->
			
			<div class="sec_three_bottom_content content">
				
				<h2 class="hack">Proin mollis justo ac enim pellentesque, non mollis sapien eleifend. Quisque placerat molestie ante ut blandit. Pellentesque euismod lobortis nisl, id bibendum risus molestie nec.</h2>
				
				<p>Duis ligula velit, cursus vitae varius quis, rutrum eget nibh. Orci varius natoque penatibus et magnis dis parturient montes, nascetur ridiculus mus. Nam accumsan convallis lorem in rhoncus. Praesent at vulputate lorem. Morbi et elementum velit. Aenean porttitor euismod est, ut sollicitudin enim. Duis pharetra velit vitae odio volutpat pellentesque. Donec faucibus porta varius. Nunc sit amet laoreet velit. Nam urna dui, ornare ut venenatis vulputate, pellentesque aliquam nibh. Sed et ligula at nibh interdum sodales. Maecenas porttitor consectetur justo, vitae aliquet turpis vestibulum eget.</p>


				<h2>Quisque convallis sem at lectus tincidunt, sit amet euismod eros finibus. </h2>
				
				<p>Duis ligula velit, cursus vitae varius quis, rutrum eget nibh. Orci varius natoque penatibus et magnis dis parturient montes, nascetur ridiculus mus. Nam accumsan convallis lorem in rhoncus. Praesent at vulputate lorem. Morbi et elementum velit. Aenean porttitor euismod est, ut sollicitudin enim. Duis pharetra velit vitae odio volutpat pellentesque. Donec faucibus porta varius. Nunc sit amet laoreet velit. Nam urna dui, ornare ut venenatis vulputate, pellentesque aliquam nibh. Sed et ligula at nibh interdum sodales. Maecenas porttitor consectetur justo, vitae aliquet turpis vestibulum eget.</p>
				
			</div><!-- sec_three_bottom_content -->
			
		</div><!-- sec_three_bottom_content_wrapper -->
		
	</div><!-- sec_three_inner -->
	
</section><!-- section_three -->